...
if st.session_state["authenticated"]:
    ...
    def count_tokens(text):
        response = openai.Completion.create(model="text-davinci-002", messages=[{"role": "system", "content": text}])
    ...
    if selected_option == "選択してください":
        ...
        initial_prompt = (
                    ...
                    "以下の指示1から指示3に従って作業を行ってください。出力は下記の「形式」に従いmarkdown形式とし、「#指示」の文言は出力しないでください。また、「Pictet」という単語は必ず「ピクテ」と翻訳してください。\n"
                    "＃指示1\n"
        ...
        initial_prompt = (
            ...
            "形式: \n"
            "・「〇〇」→「〇〇」 \n"
            "出力3:下記の例にあるような表記揺れ（同音・同義の語句について異なる文字表記が付されること）があるものは、該当する語句を全て、その下に示す形式で出力してください。その際、表記揺れと考えた理由は出力しないでください。 \n"
            "【例】 \n"
            "（1）送り仮名による表記ゆれ \n"
