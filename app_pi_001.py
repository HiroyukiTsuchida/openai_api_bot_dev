...
if st.session_state["authenticated"]:
    ...
    if selected_option == "選択してください":
        ...
        initial_prompt = (
                    ...
                    "以下の「指示」に従って翻訳してください。出力は下記の「形式」に従いmarkdown形式とし、「#指示」の文言は出力しないでください。\n"
                    "＃指示\n"
        ...
        if choice == "直接入力":
            # session_stateの更新
            if "user_input_summarize" in st.session_state:
                default_value = st.session_state["user_input_summarize"]
            else:
                default_value = ""
            # ウィジェット生成
            user_input = st.text_area("要約したい文章を入力してください。", value=default_value, height=500, key="user_input_summarize")
        ...
        # ファイルアップロードが選択された場合
            ...
            if uploaded_file is not None:
                ...
                st.session_state["user_input_translation"] = extracted_text
                # ウィジェット生成
                user_input = st.text_area("PDFから抽出したテキスト:", value=extracted_text, key="user_input_summarize", height=500)
        ...
        initial_prompt = (
                    ...
                    "###\n"
                    "＃指示2\n"
                    "#入力された文章中、固有名詞にあたるものを下記の例に従ってリスト化し、【固有名詞】として出力してください。\n"
                    "#例\n"
                    "・固有名詞１（○段落○行目）\n"
                    "＃指示3\n"
                    "#入力された文章中、数値にあたるものを下記の例に従ってリスト化し、数値の説明とともに【数値とその説明】として出力してください。\n"
                    "#例\n"
                    "・○○％（○段落○行目）：○○の割合\n"
                    "###\n"
                    "#形式\n"
        ...
        initial_prompt = (
            ...
            "なお、処理1と処理2については結果を文章として出力しないでください。  \n"
            "出力1「修正箇所リスト」の出力と出力2「修正後全文」のみ行ってください。  \n"
            "あなたの役割は、日本の投資家向けに公表される情報を校閲・校正し、間違いなく高品質な文章を作成することです。  \n"
            "これから入力する文章に対して、下記の操作1を行い、出力してください。 \n"
            "なお、出力の際は、行った操作の説明などは出力せず、指摘事項と修正後の全文のみ出力してください。  \n"

            "処理1:誤りの検知（出力しない）
            "以下の基準に該当するものをすべて検知してください。検知された個所は、どんなに大量になっても構いません。  \n"
            "誤りかどうかの判断が微妙な、微細な誤りと思われるものも含め、少しでも違和感があるものは、  \n"
            "積極的にどんどん検知し、一つの漏れもなく、すべてリスト化してください。  \n"
            ...
            "【例】  \n"
            "×受付け件数→〇受付件数、×売上げ高→〇売上高、×卸売り問屋→〇卸売問屋、×支払い総額→〇  \n"
             ...
             "#9:「・・・」や”・・・”がきちんと閉じられていないもの（「や"で始まっているのに、終わりの」や"がないものや、逆に終わりの」や"があるのに始まりの「や"がないもの）  \n"

             "#10:話し言葉等の正式な文章にふさわしくない表現になっているもの  \n"

             "【例】  \n"
             "「ちゃんと」や「ちょっと」、「食べれない」（「食べられない」の「ら」が抜けた「ら抜き言葉」）、「怒ってる」（「怒っている」の「い」が抜けた「い抜き言葉」）、「読まさせる」や「聞かさせる」（「読ませる」「聞かせる」に本来不要な「さ」が入った「さ入れ表現」）  \n"

             "#11:重複表現になっているもの  \n"
             "【例】  \n"
             "「一番最初」、「後で後悔する」、「必ず必要」、「被害を被る」  \n"
             ...
             "出力1:修正箇所リスト  \n"
             "操作2で修正した指摘したすべての個所について、以下に示す箇条書きの形式で、一つずつ改行して出力してください。  \n"
             "形式:  \n"
             "・「〇〇」→「〇〇」  \n"
            ...
            #"- 修正3:数字の表記は、全て半角とします。全角の数字があった場合は全て指摘してしてください。  \n"
            ...
            #"- 文章を省略しない。
            #"- 指摘した個所は・「〇〇」→「〇〇」と箇条書き形式で出力してください。また、該当がない修正は出力しないでください。   \n\n"
            #"### 操作2:\n"
